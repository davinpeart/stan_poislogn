poisson_lognormal2 <- function(predict_ancillary, repeated_measures, priors,
...
", if(!is.null(predict_ancillary) & multiple_matrices) {
...
// random effects
  ...
  parameters <- paste0(
    ...
    ", add_fixef(predict_ancillary, full = multiple_matrices, "phi", T),
...
// likelihood
    for(n in 1:N) {  // normal hyperprior on means
      target += normal_lpdf(mu[n] | X[n, ] * beta", add_ranef(repeated_measures, 1),
                       ", exp(", add_fixef(predict_ancillary, full = multiple_matrices, "phi"),
...
"// replications for prior predictive checks
    array[N] int y_rep; // Y replications
    for(n in 1:N) {
        y_rep[n] = poisson_log_rng(mu[n]);
    }"
    )
